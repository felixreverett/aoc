...
object Day07 {
    ...
    def partTwo(): (Int, Double) = {
        ...
        val spaceNeeded = 30000000 - (70000000 - directories("/"))

        val solution = directories.toSeq.sortBy(_._2).find(_._2 >= spaceNeeded) match {
            case Some((path, size)) => size.toInt
            case None => -1
        }
